...
public class UploadClient
{
    ...
    {
        var chunckSize = 5242880;

        int GetChunkSize(int i)
        {
            var position = (i * (long) chunckSize);
            var toRead = (int) Math.Min(file.Length - position + 1, chunckSize);
            return toRead;
        }
        ...
        var totalChunks = (int) (file.Length / chunckSize);
        ...
        for (var i = 0; i < totalChunks; i++)
        {
            ...
            // form.Add(new ByteArrayContent(buffer), "file", "blob");
            using var response = await _httpClient.PostAsync("/uploads/chunk", form).ConfigureAwait(false);
